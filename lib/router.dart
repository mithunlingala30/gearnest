...
import 'screens/web/web_categories_screen.dart';
import 'screens/web/web_login_screen.dart';
import 'screens/web/web_signup_screen.dart';
import 'screens/web/web_equipment_detail_screen.dart';
...
/// Determines whether to use the web layout.
/// Uses [kIsWeb] so this is evaluated at runtime.
bool get _isWeb => kIsWeb;

final GoRouter appRouter = GoRouter(
  initialLocation: _isWeb ? '/home' : '/splash',
  ...
  routes: [
    ...
    // On web: skip splash/onboarding — use split-panel auth directly.
    if (!_isWeb) ...[
      ...
      GoRoute(path: '/splash', redirect: (_, __) => '/home'),
      GoRoute(path: '/onboarding-1', redirect: (_, __) => '/home'),
      GoRoute(path: '/onboarding-2', redirect: (_, __) => '/home'),
      GoRoute(path: '/onboarding-3', redirect: (_, __) => '/home'),
    ],

    // Auth routes — platform-adaptive
    GoRoute(
      path: '/login',
      builder: (_, __) =>
          _isWeb ? const WebLoginScreen() : const LoginScreen(),
    ),
    GoRoute(
      path: '/signup',
      builder: (_, __) =>
          _isWeb ? const WebSignupScreen() : const SignUpScreen(),
    ),
    GoRoute(path: '/forgot-password', builder: (_, __) => const ForgotPasswordScreen()),

    // ── Main shell — platform-adaptive shell wrapper ──────────────────────────
    ShellRoute(
      builder: (context, state, child) =>
          _isWeb ? WebShell(child: child) : AppShell(child: child),
      routes: [
        GoRoute(
          path: '/home',
          builder: (_, __) =>
              _isWeb ? const WebHomeScreen() : const HomeScreen(),
        ),
        GoRoute(
          path: '/categories',
          builder: (_, __) =>
              _isWeb ? const WebCategoriesScreen() : const CategoriesScreen(),
        ),
    ...
    GoRoute(
      ...
      builder: (_, state) {
        ...
        return _isWeb
            ? WebEquipmentDetailScreen(id: id)
